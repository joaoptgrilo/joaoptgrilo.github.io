...
import { FaGithub, FaLinkedin, FaEnvelope } from 'react-icons/fa'; // These will now be used
...
const Hero: React.FC = () => {
  const socialLinks: SocialLink[] = [ // Type applied correctly
    { href: 'https://github.com/joaoptgrilo', icon: <FaGithub />, ariaLabel: 'João Grilo on GitHub' },
    { href: 'https://www.linkedin.com/in/joaoptgrilo/', icon: <FaLinkedin />, ariaLabel: 'João Grilo on LinkedIn' },
    { href: 'mailto:joao.grilo.dev@gmail.com', icon: <FaEnvelope />, ariaLabel: 'Email João Grilo' },
  ];
  ...
  return (
    ...
    >
      <div className="z-10 relative space-y-3 sm:space-y-4 md:space-y-5 max-w-3xl text-left">
        <h1
          className="font-fira-code font-bold text-4xl xs:text-4xl sm:text-5xl md:text-6xl lg:text-7xl text-primary leading-tight animate-fadeInUp tracking-tight sm:tracking-normal"
        >
          João Grilo
        </h1>
        <p
          className="font-fira-code font-semibold text-xl xs:text-xl sm:text-2xl md:text-3xl lg:text-4xl text-primary leading-snug animate-fadeInUp animation-delay-200 tracking-tight sm:tracking-normal"
        >
        ...
        <div className="flex justify-start space-x-5 xs:space-x-6 sm:space-x-7 pt-3 sm:pt-4 animate-fadeInUp animation-delay-800">
          {/* CORRECTED USAGE of socialLinks data */}
