...
import { MetricItem } from "@/data/types";

const About = ({ metricsData }: { metricsData: MetricItem[] }) => {
    ...
    <>
      <Section id="about" title="about">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-stretch mb-8 lg:mb-12">
          <AnimateOnScroll staggerDelay={0} className="h-full lg:col-span-1">
            <Panel className="h-full">
              <div className="flex flex-col items-center">
                ...
                <p className="font-fira-code text-info-accent text-sm mb-4">
                  {t("jobTitle")}
                </p>
          ...
          <AnimateOnScroll staggerDelay={150} className="h-full lg:col-span-2">
            <Panel className="h-full flex flex-col">
              <p className="font-fira-code text-xl md:text-2xl text-info-accent mb-4 text-center sm:text-left font-semibold">
                {t("summaryTitle")}
              </p>
        ...
        <AnimateOnScroll staggerDelay={300}>
          {/* MODIFIED: Changed <div> back to <Panel> to restore ref forwarding */}
          <Panel
            variant="default"
            className="backdrop-blur-custom glass-effect">
            <p className="font-fira-code text-xl md:text-2xl text-info-accent mb-8 text-center font-semibold">
              {t("metricsTitle")}
            </p>
              ...
              {metricsData.map((metric, index) => (
                <AnimateOnScroll
                  ...
                  <InfoPanel
                    ...
                    icon={
                      <Icon
                        iconId={metric.iconId as any}
                        className="w-12 h-12"
                      />
                    }
                    staggerDelay={index}
                    onPanelClick={handleShowToast}
                  />
                </AnimateOnScroll>
              ))}
            </ul>
